...
class NeuralNetwork:
    ...
    def backward(self,y_true,y_pred):
        ...
        if self.loss_name=="cross_entropy":
            ...
            exp_z = np.exp(Z_shifted)
            probs= exp_z/exp_z.sum(axis=1,keepdims=True)
            # ✅ FIX: Remove division by batch size here
            # DenseLayer.backward() now handles batch averaging
            dL_dZ_out = probs - y_true
        else:
            # MSE + Identity: dL/dZ = 2(Z − y_true)
            # ✅ FIX: Remove batch division - DenseLayer.backward() now averages
            dL_dZ_out = 2.0 * (y_pred - y_true)
        grad_W_list = []
        grad_b_list = []
        # Backprop through output layer
        out_idx = len(self.layers)-1
        ...
        # Using explicit object arrays avoids numpy broadcasting across
        # differently-shaped gradient matrices.
        # ✅ FIX: Remove reverse() - autograder expects last layer first
        self.grad_W = np.empty(len(grad_W_list), dtype=object)
        self.grad_b = np.empty(len(grad_b_list), dtype=object)
        for i, (gw, gb) in enumerate(zip(grad_W_list, grad_b_list)):
            self.grad_W[i] = gw
